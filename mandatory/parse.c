...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025-02-14 19:52:03 by yoayedde          #+#    #+#             */
/*   Updated: 2025-02-14 19:52:03 by yoayedde         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_stack	*ft_parse_em(int ac, char **av)
{
	...
	while (args1[++i])
	{
		...
		while (args1[i][k])
		{
			...
			tmp = ft_ft_lstnew(ft_atoi(args1[i][k], head));
	...
	free_mem(args1);
	if (!check_dup(head))
		return (clear_stack(head), write(2, "Error\n", 6), NULL);
	return (head);
}
